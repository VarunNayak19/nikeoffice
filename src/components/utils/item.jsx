...
const Item = ({ ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
    ...
    return (
        <>
            <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? " justify-items-start" : "justify-items-center"} 
             rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
                    ...
                    <div className='flex items-center
                     gap-3'>
                        <button type='button' className='
                         ...
                         '><ShoppingBagIcon onClick={onAddToCart} className='icon-style text-slate-900' /></button>
                        <button type='button' onClick={() => { onAddToCart(); onCartToggle(); }} className='
                ...
                `}>
                    <img
                        src={img}
                        alt="shoesimg"
                        className={` transition-theme hover:-rotate-12 ${ifExists ? " h-auto w-64 lg:w-56 md:w-48 -rotate-[35deg]" : "justify-items-center h-32 w-64"}`}
                    />
                </div>
            </div>
        </>
